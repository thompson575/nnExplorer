...
// ------------------------------------------------------------------
// Fit a NN by Full Gradient Descent with validation
// inputs
...
List rcpp_GDV(  NumericMatrix X, 
  ...
  NumericVector validHistory (nEpoch);            // validation loss
  NumericVector magHistory (nEpoch);             // MAG at each epoch
  NumericVector dpenw (nWts);                    // derivative of penalty wrt weight
  ...
  NumericVector vbias (nNodes);
  NumericVector vweight (nWts);
  ...
  if( maxvrise == 0) maxvrise = nEpoch + 1;
  ...
  for(int epoch = 0; epoch < nEpoch; epoch++) {
    ...
    if( (epoch == 0) | (epoch % validGap == 0) ) {
      ...
      for( int d = 0; d < nv; d++) {
        ...
        // calculate the loss
        vloss += rcpp_loss(y, yhat, lossFun);
      }
      validHistory[epoch] = vloss / nv;
      if( epoch > 0 ) {
        if( validHistory[epoch] > validHistory[epoch-1]) {
          vrise++;  
          if( vrise >= maxvrise ) converged = 1;
        }
        else vrise = 0;
      }
      // save parameters if an improvement
      if( (epoch == 0) | (validHistory[epoch] < minvLoss) ) {
        minvLoss = validHistory[epoch];
        for(int i = 0; i < nWts; i++)  vweight[i] = weight[i];
        for(int i = nX; i < nNodes; i++) vbias[i] = bias[i];
      }
    } else validHistory[epoch] = validHistory[epoch-1];
    // update the weights and biases
    ...
    if( ((epoch+1) % trace == 0) | ((trace < nEpoch) & (converged == 1))) {
      if( cprint % 10 == 0 ) Rprintf("  epoch       loss    penalty      valid    minLoss    minvLoss    step       MAG\n");
      Rprintf("%6i %10.5f %10.5f %10.5f %10.5f %10.5f %8.5f %10.6f\n", 
              epoch+1, lossHistory[epoch], penHistory[epoch], validHistory[epoch], minLoss, minvLoss, eta, mag); 
      cprint++;
    }
    // finished
